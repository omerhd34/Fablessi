...
import { isCssColorValue, resolveColorCss } from "@/lib/color-value";
...
export function getPrimaryImageUrl(product) {
 ...
 const images = getVariantImages(defaultVariant);
...
function attachProductImages(product) {
 ...
 return {
  ...product,
  variants,
  images: getVariantImages(defaultVariant),
 };
}
...
export function getProductDisplayPrice(product) {
 return getProductPriceTotal(product);
}
...
export function colorToImagePrefix(color, variantName) {
 if (!color) return null;
 if (isCssColorValue(color)) {
  return variantName?.trim().toLocaleLowerCase("tr") ?? null;
 }
...
export function getImagesForVariant(_images, variant) {
 return getVariantImages(variant).length > 0
  ? getVariantImages(variant)
  : (_images ?? []);
}

export function getColorSwatch(color) {
 const css = resolveColorCss(color);
 if (css) return css;
