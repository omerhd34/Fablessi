...
import { formatSeoTitle, resolveSiteHomeUrl, resolveSiteUrl } from "@/lib/site-metadata";
import {
 FEATURED_SECTION_ID,
...
export function buildOrganizationJsonLd(locale = "tr") {
 ...
 return {
  ...
  name: brandFullName,
  alternateName: [brandName, "gardossi.com"],
  description,
...
export function buildSiteStructuredDataGraph(locale = "tr") {
 return {
  "@context": "https://schema.org",
  "@graph": [buildOrganizationJsonLd(locale), buildWebSiteJsonLd(locale)],
 };
}

export function buildWebSiteJsonLd(locale = "tr") {
 const siteUrl = resolveSiteUrl();
 const siteHomeUrl = resolveSiteHomeUrl();
 const sitelinks = getGoogleSitelinks(locale).filter(
...
export function buildPageWebPageJsonLd({ pageKey, path, locale = "tr" }) {
 ...
 const metaTitle = seo.metaTitle ?? seo.absoluteTitle;
