...
export const FAVORITES_STORAGE_KEY = "gardossi:favorites";
...
export function enrichFavoriteForDisplay(favorite, productMetaBySlug = {}) {
 ...
 return {
  ...favorite,
  name: favorite.name ?? meta.name ?? favorite.slug,
...
export function favoriteToProductCard(favorite) {
 return {
  slug: favorite.slug,
  name: favorite.name ?? favorite.slug,
  ...
  images: favorite.imageUrl
   ? [{ url: favorite.imageUrl, alt: favorite.name ?? favorite.slug }]
   : [],
 };
}
